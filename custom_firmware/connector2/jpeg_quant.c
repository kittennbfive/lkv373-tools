...
//CAREFULL with serial output here! The serial port is accessed by a script (serial.pl or the connector from simulate/) that are expecting a specific binary protocol!
//(serial.pl currently adapted to see some error messages)
...
void set_q(unsigned char Q, unsigned char const * const unknown4)
{
	...
	//printf("JPEG: Q=%u unknown4 is 0x %x %x %x %x\r\n", Q, unknown4[0], unknown4[1], unknown4[2], unknown4[3]); 
	
	write_quant_tables(unknown4, luma, chroma);
...
void write_quant_tables(unsigned char const * const unknown4, unsigned char const * const luma_tbl, unsigned char const * const chroma_tbl)
{
	...
	for(i=0; i<64; i++)
	{
		...
		do
		{
			...
			*((volatile unsigned int*)0x9090c200)=unknown4[0];
			*((volatile unsigned int*)0x9090c200)=unknown4[1];
			*((volatile unsigned int*)0x9090c200)=unknown4[2];
		...
		*((volatile unsigned int*)0x9090c200)=unknown4[3];
	}
