...
static void disassm16(instr_t * const instr_struct, const uint32_t PC __attribute__((__unused__)))
{
	switch(instr_struct->type16)
	{
		...
		case TYPE16_10:
			...
			{
				case OPC16_ADDI10S:
					sprintf(instr_struct->disassm, "%s %d\n", instr_struct->mnemonic, nds32_sign_extend(instr_struct->imm1_10, 10, 32));
					break;
			}
			break;
...
/*
static uint8_t translate_to_32(instr_t * const instr_struct)
{
		...
		case 4:
			switch(instr_struct->opc)
			{
				...
				case OPC16_BEQZ38:
					...
					instr_struct->imm1_20=nds32_sign_extend(instr_struct->imm1_8, 8, 20);
					break;
